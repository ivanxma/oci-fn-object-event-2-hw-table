...
| Image | Mandatory existing `OCI_REGISTRY_REPOSITORY_ID`; setup validates it and resolves the read-only `OCI_REGISTRY_REPOSITORY`. Processor releases use increasing immutable `processor-<version>` tags; UI releases use `ui-<version>` in the same repository. Event Processor includes only Processor tags. |
