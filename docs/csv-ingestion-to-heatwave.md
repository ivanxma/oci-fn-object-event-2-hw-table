...
- **MySQL Shell is not a drop-in application API.** MySQL Shell import utilities can work with Object Storage files, avoiding the application's own temporary-file handling. However, they are Shell utilities rather than a direct in-process ingestion API. An application or Function must start, supervise, secure, and interpret a complete MySQL Shell process, including its configuration, exit status, logs, timeout, and retry behaviour. That process-level integration can be practical for batch operations, but it adds deployment and operational complexity to an event-driven application.
