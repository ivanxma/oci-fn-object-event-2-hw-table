...
class VaultSecretServiceTest(unittest.TestCase):
    ...
    def test_create_database_secret_encodes_json_without_returning_value(self):
        ...
        with patch.object(service, "_client", return_value=(oci, client)):
            result = service.create_database_secret(name="processor-db", vault_id="ocid1.vault.test", key_id="ocid1.key.test", host="10.0.0.8", port="3306", user="streamuser", credential="secret-value", database="default_db", control_database="stream_db", stream_data_database="stream_data", staging_database="staging_db")
        self.assertEqual(result.id, "ocid1.vaultsecret.new")
        payload = json.loads(base64.b64decode(created[0].secret_content.content))
        self.assertEqual(payload, {"host": "10.0.0.8", "port": 3306, "user": "streamuser", "credential": "secret-value", "database": "default_db", "control_database": "stream_db", "stream_data_database": "stream_data", "staging_database": "staging_db"})
