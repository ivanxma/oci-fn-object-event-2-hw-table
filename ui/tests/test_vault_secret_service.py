...
class VaultSecretServiceTest(unittest.TestCase):
    ...
    def test_create_database_secret_encodes_json_without_returning_value(self):
        ...
        with patch.object(service, "_client", return_value=(oci, client)):
            result = service.create_database_secret(name="processor-db", vault_id="ocid1.vault.test", key_id="ocid1.key.test", host="10.0.0.8", port="3306", user="streamuser", credential="secret-value", database="testdb", control_database="stream_db", stream_data_database="stream_data")
    ...
    def test_create_database_secret_rejects_invalid_ocids_before_oci_call(self):
        ...
        with self.assertRaisesRegex(ValueError, "Vault and encryption key"):
            service.create_database_secret(name="processor", vault_id="no", key_id="no", host="db", port="3306", user="u", credential="x", database="d", control_database="c", stream_data_database="s")
