...
class ImportService:
    ...
    def generated_sql(self, database: str, table: str, columns: list[dict], primary_key: list[str], add_row_id: bool, delimiter: str, *, create_database: bool, partition_by_batch: bool, include_data: bool, drop_existing: bool) -> str:
        """Return the complete reviewed import script without executing it.

        The browser upload is not a database-server path, therefore the data
        statement intentionally uses an operator-supplied client path marker.
        """
        statements: list[str] = ["-- Generated by Import File. Review before execution."]
        if create_database:
            statements.append(self.create_database_statement(database) + ";")
        if drop_existing:
            statements.append(f"DROP TABLE IF EXISTS {quote_identifier(database, 'database name')}.{quote_identifier(table, 'table name')};")
        statements.append(self.ddl(database, table, columns, primary_key, add_row_id, partition_by_batch) + ";")
        if include_data:
            statement = self.load_data_statement(database, table, columns, delimiter, partition_by_batch)
            statements.append("-- Replace __LOCAL_CSV_PATH__ with a client-accessible CSV path.")
