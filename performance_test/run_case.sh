...
container_python() {
  local script=$1
  shift
  sudo --preserve-env=DB_HOST,DB_PORT,DB_USER,DB_CREDENTIAL,CONTROL_DATABASE,TARGET_DATABASE,TARGET_TABLE,COMPARTMENT_NAME,BUCKET_NAME,RESOURCE_NAME_PATTERN,INVOCATION_MODE,WRITER_WORKERS,OBJECT_NAME,EXPECTED_ACTION,EXPECTED_ROWS,EVENT_WAIT_SECONDS,FILE_BYTES,CASE_LABEL,PAYLOAD_BYTES,GENERATION_SECONDS,UPLOAD_SECONDS \
    ...
    -v "$ROOT_DIR/performance_test/$script:/opt/$script:ro,Z" \
