#!/usr/bin/env bash
# Build and push the mode-aware OCI Streaming consumer image to OCIR.
...
for value in REGION_KEY REPOSITORY_PREFIX CONSUMER_IMAGE_NAME CONSUMER_IMAGE_TAG; do
...
docker build --file "$ROOT_DIR/consumer/Dockerfile" --tag "$IMAGE" "$ROOT_DIR"
docker push "$IMAGE"
echo "Consumer image pushed: $IMAGE"
