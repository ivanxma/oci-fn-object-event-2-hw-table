...
jq -n --arg host "$DB_HOST" --arg port "${DB_PORT:-3306}" --arg user "$DB_USER" --arg password "$DB_PASSWORD" --arg ssl "${DB_SSL_DISABLED:-false}" --arg namespace "${OBJECT_STORAGE_NAMESPACE:-}" --arg batch "${BATCH_ROWS:-10000}" --arg workers "${WRITER_WORKERS:-4}" --arg read_timeout "${OBJECT_STORAGE_READ_TIMEOUT_SECONDS:-300}" --arg range_bytes "${OBJECT_STORAGE_RANGE_BYTES:-33554432}" --arg control "$CONTROL_DATABASE" '{DB_HOST:$host,DB_PORT:$port,DB_USER:$user,DB_PASSWORD:$password,DB_SSL_DISABLED:$ssl,OBJECT_STORAGE_NAMESPACE:$namespace,BATCH_ROWS:$batch,WRITER_WORKERS:$workers,OBJECT_STORAGE_READ_TIMEOUT_SECONDS:$read_timeout,OBJECT_STORAGE_RANGE_BYTES:$range_bytes,CONTROL_DATABASE:$control}' > "$CONFIG_FILE"
