...
if [[ "$OCI_CONTAINER_ORCHESTRATION_ENABLED" == "true" ]]; then
  for value in SUBNET_ID CONTAINER_AVAILABILITY_DOMAIN PROCESSOR_SHAPE PROCESSOR_OCPUS PROCESSOR_MEMORY_GBS PROCESSOR_IMAGE_URL DB_SECRET_OCID; do
...
printf 'STAGING_DATABASE=%s\n' "${STAGING_DATABASE:-stream_staging}" >> "$RUNTIME_ENV"
