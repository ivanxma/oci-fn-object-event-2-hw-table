...
if [[ "$OCI_CONTAINER_ORCHESTRATION_ENABLED" == "true" ]]; then
  for value in SUBNET_ID CONTAINER_AVAILABILITY_DOMAIN PROCESSOR_SHAPE PROCESSOR_OCPUS PROCESSOR_MEMORY_GBS PROCESSOR_IMAGE_URL DB_SECRET_OCID DB_HOST DB_PORT DB_USER DB_NAME STREAM_DATA_DB_NAME; do
...
printf 'FLASK_SECRET_KEY=%s\nCONTROL_DATABASE=%s\nSESSION_COOKIE_SECURE=1\nOCI_EVENT_RULE_MANAGEMENT_ENABLED=%s\nOCI_STREAMING_MANAGEMENT_ENABLED=%s\nOCI_CONTAINER_ORCHESTRATION_ENABLED=%s\nOCI_EVENT_RULE_PREFIX=%s\nOCI_COMPARTMENT_ID=%s\nOCI_REGION=%s\nOCI_OBJECT_STORAGE_NAMESPACE=%s\nSUBNET_ID=%s\nCONTAINER_AVAILABILITY_DOMAIN=%s\nPROCESSOR_SHAPE=%s\nPROCESSOR_OCPUS=%s\nPROCESSOR_MEMORY_GBS=%s\nPROCESSOR_IMAGE_URL=%s\nPROCESSOR_CONTAINER_NAME_PREFIX=%s\nWRITER_WORKERS=%s\nDB_SECRET_OCID=%s\nDB_HOST=%s\nDB_PORT=%s\nDB_USER=%s\nDB_NAME=%s\nSTREAM_DATA_DB_NAME=%s\n' \
  "$FLASK_SECRET_KEY" "$CONTROL_DATABASE" "$OCI_EVENT_RULE_MANAGEMENT_ENABLED" "$OCI_STREAMING_MANAGEMENT_ENABLED" "$OCI_CONTAINER_ORCHESTRATION_ENABLED" "$OCI_EVENT_RULE_PREFIX" "${COMPARTMENT_ID:-}" "${REGION:-}" "${OBJECT_STORAGE_NAMESPACE:-}" "${SUBNET_ID:-}" "${CONTAINER_AVAILABILITY_DOMAIN:-}" "${PROCESSOR_SHAPE:-}" "${PROCESSOR_OCPUS:-}" "${PROCESSOR_MEMORY_GBS:-}" "${PROCESSOR_IMAGE_URL:-}" "${PROCESSOR_CONTAINER_NAME_PREFIX:-object-storage-stream-processor}" "${WRITER_WORKERS:-4}" "${DB_SECRET_OCID:-}" "${DB_HOST:-}" "${DB_PORT:-3306}" "${DB_USER:-}" "${DB_NAME:-}" "${STREAM_DATA_DB_NAME:-}" > "$RUNTIME_ENV"
