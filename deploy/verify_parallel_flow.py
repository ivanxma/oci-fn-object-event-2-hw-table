#!/usr/bin/env python3
"""Disposable Object Storage -> two-partition Stream -> processor verification."""
...
class ParallelVerification:
    def __init__(self) -> None:
        ...
        self.stream_id = required("PARALLEL_STREAM_ID")
        ...
        self.prefix = os.environ.get("PARALLEL_RESUME_PREFIX", "").strip() or f"parallel-verify-{suffix}"
        self.target_database = identifier(required("PARALLEL_TARGET_DATABASE"), "parallel target database")
        self.target_table = identifier(
            os.environ.get("PARALLEL_RESUME_TARGET_TABLE", "").strip() or f"employees_parallel_{suffix}",
            "parallel target table",
        )
        resume_mapping = os.environ.get("PARALLEL_RESUME_MAPPING_ID", "").strip()
        self.mapping_id: int | None = int(resume_mapping) if resume_mapping else None
        self.rule_id = os.environ.get("PARALLEL_RESUME_RULE_ID", "").strip()
        self.container_ids = [
            value.strip()
            for value in os.environ.get("PARALLEL_RESUME_CONTAINER_IDS", "").split(",")
            if value.strip()
        ]
        self.object_names = (
    ...
    def verify_contract(self) -> None:
        ...
        if str(stream.lifecycle_state).upper() != "ACTIVE" or int(stream.partitions) != 2:
            raise RuntimeError("PARALLEL_STREAM_ID must identify an ACTIVE two-partition Stream.")
    ...
    def create_mapping_and_rule(self) -> None:
        ...
        action = oci.events.models.CreateStreamingServiceActionDetails(
            action_type="OSS",
            is_enabled=True,
            description="Disposable two-partition processor verification",
            stream_id=self.stream_id,
        )
        details = oci.events.models.CreateRuleDetails(
            display_name=f"parallel-verify-mapping-{self.mapping_id}-{self.prefix[-6:]}",
    ...
    def upload(self) -> None:
        for index, content in enumerate(split_csv(self.fixture), start=1):
            name = f"{self.prefix}/employees{index:02d}.csv"
            self.object_storage.put_object(self.namespace, self.bucket, name, content)
            self.object_names.append(name)
        print(f"PASS: uploaded {len(self.object_names)} mutually exclusive CSV objects")
    ...
    def wait_for_create_flow(self) -> dict[str, Any]:
        def complete():
            ...
            if (
                ...
                and int(value["partitions"] or 0) == 2
                and int(value["rows_loaded"] or 0) == 50
        ...
        return wait_until("parallel create flow and transaction log", complete)
    ...
    def delete_subset_and_verify(self) -> dict[str, Any]:
        ...
        def complete():
            ...
            if (
                ...
                and int(value["failed"] or 0) == 0
                and int(value["rows_loaded"] or 0) == 25
                and int(value["target_partitions"] or 0) == 6
            ):
                return value
            return False

        return wait_until("five DELETE events drop five owned partitions", complete)
    ...
    def delete_remaining_and_verify(self) -> None:
        ...
        wait_until("remaining DELETE events leave only seed partition", complete)
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            connection.close()
        print("PASS: disposable Parallel verification resources cleaned up")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if self.mapping_id is None:
                ...
                self.create_processor(0)
                self.create_processor(1)
                self.upload()
            else:
                if not self.rule_id or len(self.container_ids) != 2:
                    raise ValueError(
                        "A resumed run requires PARALLEL_RESUME_RULE_ID and two PARALLEL_RESUME_CONTAINER_IDS."
                    )
                print(
                    f"PASS: resuming preserved Parallel verification mapping {self.mapping_id}",
                    flush=True,
                )
            ...
            print(
                "PASS: Parallel flow verified "
                f"(create={created}, after-five-deletes={deleted})",
                flush=True,
            )
            succeeded = True
        finally:
            keep_on_failure = os.environ.get("PARALLEL_KEEP_RESOURCES_ON_FAILURE", "true").lower() == "true"
